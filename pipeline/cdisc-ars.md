...
生成した JSON は、モデルから導かれた JSON-Schema で検証する。試験Aでは当初これを行わず、必須スロットを数個だけ見る検査を自前で書いて準拠と判断していた。スキーマにかけたところ6種類・1,319件の必須スロット漏れが出た。自前の検査は書いた分しか見ない。内訳は、結果を群ごとに報告するかの真偽値、図表の表示の並び、集団と部分集合の階層と順序、群がデータ由来かの真偽値、目次が入れ子の型であること。いずれも値の誤りではなく、モデルが要求するスロットを出していなかったものである。

スキーマの写しは試験リポジトリの `docs/metadata/external/` に置き、網に出て取り直さない。実行のたびに検査の結果が変わらないようにするためで、標準が改訂されたら写しを更新して差分を見る。検証には JSON-Schema の実装が要るので、Python を標準ライブラリだけで動かす方針の例外になる。入っていない環境では合否と区別できる終了コードで「検証できなかった」と返し、黙って通さない。
...
出すと決めたときに要る作業は多くない。試験Aの実績では、識別子の体系（解析ID・図表ID・母集団・部分集団・手法ID・操作ID・群と水準・結果値）が既に対応していた。試験の側で判断が要ったのは `Analysis` の `purpose`（3値から選ぶ）と `reason`（4値から選ぶ）と `name` の3つだけで、`purpose` と `reason` は解析仕様インデックスの根拠の記述から機械的に決まる。これらは機械可読な CSV を正本にし、人間可読の仕様書からは参照する。

残りの不足はスキーマにかければ機械的に出る。試験Aでは6種類あり、いずれも構造上の必須スロット（真偽値・順序・階層・入れ子の型）で、試験の側の判断を要さなかった。だからこそ、要約から必須スロットを書き起こす作りにすると漏れる。最初からスキーマ検証を組んでおけば、この6種類は実装の途中で出る。
...
- 2026-08-29：標準の JSON-Schema による検証を実施し、自前の必須スロット検査では6種類・1,319件の漏れを見落としていたことを反映。一次情報（GitHub の LinkML モデルと生成されたモデル文書）の当たり方を追記。「ARS 準拠かは判定できる問いではない」という誤りを正し、判断軸を置き直した。試験Aで ARS v1.0 準拠の ReportingEvent を両系統で定常的に作り、その形式で突合して納品へ同梱するところまで実施した実績を反映（解析1,428件・結果値29,259件・図表88件）。ReportingEvent の位置づけ（末端から枝分かれする成果物であってパイプラインの部品ではない）と、生成を1言語に閉じる理由を追記。
