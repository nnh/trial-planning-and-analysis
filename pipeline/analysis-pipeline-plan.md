...
必須2の「最終レビューと納品承認」に入る前に、レビュー対象を優先順位付けして提示する段階を置く。生成されるファイル（コード・図表・仕様書・Dataset-JSON 等）は数百に及ぶため、統計解析責任者が全件を同じ深さで見ることは非現実的である。優先順位付けが無いと、レビューの深さが見た順番に依存してしまう。
...
図表を「表ごとに1本のプログラム」で書かない。表示の作法（列幅・見出し・脚注・セル台帳への記録）を表示型のマクロ・関数に閉じ、個々の図表は CSV の1行で表す。この行が持つのは3つだけである。どの表示型で描くか、どの解析の結果値（ARD の `analysis_id`）を使うか、どの並び（群・水準）で出すか。この行を「宣言」と呼ぶ（用語の検討経緯は [declaration-terminology-research.md](declaration-terminology-research.md)）。列の完全な定義は [../templates/README.md](../templates/README.md)「tlf-index.csv」を参照。
