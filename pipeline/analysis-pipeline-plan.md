...
**同期の方向は常に試験側から枠組みへとする。** 改良は実際の試験を回す中で生まれるので、枠組みが単独で先に進むことは考えにくい。枠組みの側で書き換えるのは、試験名を伏せる・試験ごとに変わる値を設定から読む形にする、といった汎用化に限る。

そのため単純なコピーでは汎用化を巻き戻す。同期の手順は次のとおり。
...
2 で何も出ないファイルは、そのままコピーしてよい。汎用化の実装が入っているファイルは、差分を1件ずつ判定する作業になる。
