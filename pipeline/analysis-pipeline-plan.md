...
- 納品物 — 研究者へ渡すパッケージ。R のプログラム、図表、Dataset-JSON、仕様書、追跡索引
...
flowchart TD
  ...
  G1 -->|出ない| S0["SDTM の作成（2系統・独立）<br/>define.xml(SDTM) の更新と適合性検証"]:::auto
  H1 --> S0
  ...
  H3 --> S4
  G3 -->|残らない| S6["追跡索引・仕様書HTML・define.xml(ADaM)・納品パッケージ"]:::auto

  S6 --> S7["パッケージの検査<br/>参照先の実在・同梱漏れ・リンク切れ"]:::auto
  S7 --> S8["レビュー対象の優先順位付け<br/>図表・コード・仕様書を優先順位順に列挙"]:::auto
  S8 --> H4["学術レビューと納品承認"]:::human
  H4 --> OUT["研究者へ納品"]:::auto
```

桃色が人の介入、黄色が条件付きの停止、青が自動で流れる箇所である。
...
2. 最終図表の学術的レビューと納品承認。数値だけでなく臨床的・統計的に妥当な表現かの確認と、研究者へ出す責任判断
...
必須2の「最終図表の学術的レビューと納品承認」に入る前に、レビュー対象を優先順位付けして提示する段階を置く。生成されるファイル（コード・図表・仕様書・Dataset-JSON 等）は数百に及ぶため、統計解析責任者が全件を同じ深さで見ることは非現実的である。優先順位付けが無いと、レビューの深さが見た順番に依存してしまう。

Claude へ次のように指示する。

> 現時点で統計解析責任者が目視で確認していくべき書類を優先順位順に列挙してください。md ファイルだけでなく、図表やソースコードなど全てのファイルを含みます。
...
この段階はまだ1試験でしか実施していないため、機械検査や規則には昇格させず、指示文と基準だけを残す。2試験目で同じ優先順位付けを行ったら、機械的に判定できる部分（突合の不一致件数、指摘の有無等を自動で集計して優先順位の根拠に使う）を抽出できるか見る（[../findings/README.md](../findings/README.md)「昇格の判定」）。
...
図表を「表ごとに1本のプログラム」で書かない。表示の作法（列幅・見出し・脚注・セル台帳への記録）を表示型のマクロ・関数に閉じ、個々の図表は「どの表示型で、どの解析の結果値を、どの並びで描くか」を書いた CSV の1行で表す。この1行を宣言と呼ぶ。

宣言の正本は CSV 1つにし、SAS の描画・R の描画・追跡索引の生成が同じ CSV を読む。宣言の実体を2つ持つと、照合という作業と、それを行うスクリプトが必要になる。照合は手動になりやすく、壊れても気づかない。試験A では、プログラムのマクロ呼び出しと CSV の両方に宣言を持たせていた時期があり、両者を突き合わせる照合スクリプトが符号化の移行で動かなくなっていたことに、しばらく気づかなかった。宣言の実体を CSV 1つにし、照合という作業ごと無くしてこの構図自体を解消した。
...
図表のセル1つから、解析ID・ADaM の変数・SDTM の変数・症例報告書の入力欄・注釈付き CRF まで辿れる HTML を作る。描画の際にセル台帳を書き出しておけば、追跡索引の入力になると同時に、図表の層の突合にも使える。
...
- R 環境。`{admiral}`（SDTM から ADaM）、`{cards}`／`{cardx}`（ARD 生成）、`{gtsummary}`／`{gt}`／`{flextable}`（表の描画）、`{survival}`（生存解析）。バージョンを `renv` で固定し、研究者が受領後に同じ環境で再実行・拡張できるようにする
- CDISC CORE（SDTM の適合性検証。[sdtm-conformance-validation.md](sdtm-conformance-validation.md)）
- Python。追跡索引・仕様書 HTML・納品パッケージの生成と、各種の検査
...
ある試験の11本を調べたところ、試験固有だったのは2つの値だけだった。試験の識別子（プログラム名の接頭辞・納品パッケージ名・表題に使う）と、データの置き場（Box のルートからの相対パス）である。5本は元から試験に依存しておらず、残る6本も参照していたのはこの2つだけだった。

- 依存なし — 仕様書 HTML の生成、症例報告書の項目と SDTM の対応の生成と検査、変数の一覧の検査、追跡索引の検査
- 識別子だけ — 追跡索引の生成、図表の宣言の検査、納品パッケージの生成と検査
- 置き場だけ — パス解決（Python・PowerShell の2本）

設定を差し替えれば同じスクリプトが別の試験で動く。汎用化のたびに、改変の前後で成果物が1バイトも変わらないことを機械で確かめる。

生成物どうしに依存があるので、生成の順序を決めておく。追跡索引は仕様書 HTML を読むため、仕様書を先に作る。順序を決めずに回すと、1回目と2回目で結果が変わり、差分を見た人が原因を追うことになる。
...
- `label-and-traceability-design.md` — 表示文言と追跡索引の設計
...
- 図表：90の宣言（表86・図4）を12種類の表示型で描く。宣言の正本は CSV 1つで、SAS・R・追跡索引が読む
- 解析：1419件。セル台帳13872セル
- 突合：ARD は全行一致。図表は日英・両系統の4通りで86図表・不一致0・行数の差0
- 納品：441ファイル。同梱の検査でエラー0・警告0。追跡索引は図表90件すべてを解決
