...
- 受領データ — データセンターから受け取る CSV 等。SDTM 風の構造を持つが派生変数を欠くことが多い
...
受領データが SDTM 風であっても、SDTM 層を省いて ADaM を作らない。派生の起点が複数箇所に散ると、同じ計算が層をまたいで重複し、いずれずれる。
...
flowchart TD
  ...
  H3 --> S4
  G3 -->|残らない| S6["追跡索引・仕様書HTML・define.xml・納品パッケージ"]:::auto

  S6 --> S7["パッケージの検査<br/>参照先の実在・同梱漏れ・リンク切れ"]:::auto
  S7 --> H4["学術レビューと納品承認"]:::human
  H4 --> OUT["研究者へ納品"]:::auto
```
...
- define.xml（変数メタデータ）。値域・コードリストの確認に使う
...
スクリプトは3本に分ける。受領データから図表までの本流、Dataset-JSON の後処理、define.xml の更新と適合性検証。本流の中には依存のある並びがあるので（ARD の後にセル台帳、ADaM の後に JSON）、任意に並べ替えられる形にしない。
...
- Dataset-JSON（SDTM・ADaM）と define.xml
- 作成仕様の HTML（節ごとに錨を持つ）
...
- 実行：本流12段階を1本のスクリプトで回す。Dataset-JSON の後処理と define.xml の更新・適合性検証は別
