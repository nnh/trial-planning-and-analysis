...
層の分け方に加えて、実装で守る原則が6つある。いずれも実際の試験で実装し、効果を確認したもの。
...
実装ができてから基準を書くと、基準が実装の写しになる。写しは実装の誤りをそのまま承認する。基準を作る者と実装する者を分けられないなら、少なくとも作る順序を分け、基準を先に固定して版を切る。
...
`docs/metadata/`（機械が読む正本）。図表の宣言、表示文言のカタログ、変数の一覧、評価時点の対応表、参考併記の文献値と表の構造、コードリスト、適合性指摘の仕分け。CDISC Library・CT から引いた写しは `metadata/external/`、SDTM Trial Design の入力は `metadata/trial-design/` に分ける。
