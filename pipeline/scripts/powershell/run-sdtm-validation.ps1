...
#   4. CDISC CORE による検証
#
# 前提：SDTM データセットが Box の input/sdtm に出来ていること
#       （program/<試験ID>_CSVtoSDTM.sas を先に実行する）
#       CDISC CORE が %USERPROFILE%\opt\cdisc-core\core に入っていること
#       方法論の正本は ../../sdtm-conformance-validation.md
#
# 使い方：pwsh -File scripts/run-sdtm-validation.ps1 -SrcDir 'input\rawdata\<受領日フォルダ>\<define.xml のあるフォルダ>'
...
& python $igMeta --out (Join-Path $repo 'docs\sdtmig-3-2-variable-roles.csv') `
         --domains-from (Join-Path $box 'input\sdtm\sdtm_variables.csv') | Select-Object -Last 2 | Write-Host
...
& python $ctMeta --out (Join-Path $repo 'docs\ct-domain-ccode.csv') --codelist C66734 |
  Select-Object -Last 1 | Write-Host
& pwsh -File (Join-Path $repo 'scripts\update-define-xml.ps1') -SrcDir $SrcDir | Select-Object -Last 5 | Write-Host

Write-Host '   define.xml の HTML 化'
$xslt = New-Object System.Xml.Xsl.XslCompiledTransform
$xslt.Load((Join-Path $box 'input\sdtm\define2-0-0.xsl'),
           (New-Object System.Xml.Xsl.XsltSettings($true, $true)),
           (New-Object System.Xml.XmlUrlResolver))
$xslt.Transform((Join-Path $box 'input\sdtm\define.xml'), (Join-Path $box 'input\sdtm\define.html'))
Write-Host "   $(Join-Path $box 'input\sdtm\define.html')"

Write-Host '3. Dataset-JSON の生成'
Invoke-Sas -Program (Join-Path $repo "program\${trialId}_SDTMtoJSON.sas") `
           -Tag 'SDTMtoJSON' -Encoding $Encoding | Out-Null

# SAS の encoding='utf-8' は BOM を書き出すが、CORE の JSON パーサは BOM 付きを読めない
$n = 0
foreach ($f in Get-ChildItem (Join-Path $box 'input\sdtm\json') -Filter *.json) {
...
Copy-Item (Join-Path $box 'input\sdtm\define.xml') (Join-Path $box 'input\sdtm\json\define.xml') -Force
...
$files = Get-ChildItem (Join-Path $box 'input\sdtm\json') -Filter *.json
...
& $core validate -s sdtmig -v 3-2 -d (Join-Path $box 'input\sdtm\json') `
...
# 仕分けの正本は docs/core-issue-disposition.csv。既知として残すと決めたルールを別枠に
...
$disp = @{}
$dispPath = Join-Path $repo 'docs\core-issue-disposition.csv'
if (Test-Path $dispPath) { foreach ($r in Import-Csv $dispPath) { $disp[$r.core_id] = $r } }
